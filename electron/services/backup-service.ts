...
export function createBackup(
  ...
  appendBackupLog({ kind, destination: destPath, filesizeBytes, status: 'success', errorMessage: null })
...
export function readRestoreInfo(zipPath: string): RestoreInfo {
  ...
  return { version: meta.version, createdAt: meta.createdAt, tradeCount: meta.tradeCount ?? 0, accountCount: meta.accountCount ?? 0 }
}
...
export function restoreFromBackup(zipPath: string): void {
  ...
  try { createPreOpBackup() } catch { /* non-fatal */ }
...
function appendBackupLog(entry: {
  ...
  try {
    getDb().insert(schema.backupLog).values({
      id: uuidv7(),
      kind: entry.kind,
