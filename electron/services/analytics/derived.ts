 ...
 */
export function getTimeOfDayHeatmap(
  rows: TradeRowForDerived[],
  timeZone: string,
): HourDayCell[] {
  interface Acc { sumR: Decimal; n: number }
...
export function getDowSummary(
  rows: TradeRowForDerived[],
  timeZone: string,
): DowSummaryRow[] {
  interface Acc { n: number; wins: number; sumR: Decimal; sumPnl: number }
