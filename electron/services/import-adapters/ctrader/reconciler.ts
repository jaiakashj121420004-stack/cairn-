...
function reconcileClosedGroup(
  ...
  const totalVolume = sorted
    .reduce((s, r) => s + safeFloat(r.volumeLots), 0)
    .toFixed(2)

  // Total gross P&L = sum of all rows' grossProfit
  const totalPnl = sorted
    .reduce((s, r) => s + safeFloat(r.grossProfit), 0)
    .toFixed(2)

  // Total commission = sum across all rows
  const totalComm = sorted
    .reduce((s, r) => s + safeFloat(r.commission), 0)
    .toFixed(2)

  const totalSwap = sorted
    .reduce((s, r) => s + safeFloat(r.swap), 0)
    .toFixed(2)
