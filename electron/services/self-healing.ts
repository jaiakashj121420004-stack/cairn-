  ...
  try {
    ...
    for (const row of rows) {
      if (KNOWN_BOOLEAN_SETTINGS.includes(row.key)) {
        ...
        const parsed = JSON.parse(row.value)
        if (typeof parsed !== 'string' && parsed !== null) throw new Error(`Invalid string: ${row.key}`)
      }
    }
...
export function cleanupOrphanedScreenshots(): void {
  try {
    ...
    const dbFilenames = new Set(
      db.select({ filename: schema.tradeScreenshots.filename }).from(schema.tradeScreenshots).all().map((r) => r.filename),
    )
...
export function checkTimezoneChange(mainWindow: BrowserWindow | null): void {
  try {
    ...
    db.insert(schema.settings)
      ...
      .onConflictDoUpdate({ target: schema.settings.key, set: { value: JSON.stringify(current), updatedAt: Date.now() } })
      .run()
  } catch (err) {
