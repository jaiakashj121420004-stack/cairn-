...
export function registerNotebookHandlers(): void {
  ...
  ipcMain.handle('notebook:search', (_e, raw: NotebookSearchInput): IpcResponse<NotebookEntrySummary[]> => {
      ...
      const rows = db
        ...
        .where(
          and(
            ...
            or(
              like(schema.notebookEntries.title, q),
              like(schema.notebookEntries.content, q),
            ),
          ),
        )
        .orderBy(desc(schema.notebookEntries.pinned), desc(schema.notebookEntries.updatedAt))
