...
export function registerPairHandlers(): void {
  ...
  ipcMain.handle('pairs:create', (_e, raw: CreatePairInput): IpcResponse<Pair> => {
    ...
    try {
      ...
      const displayOrder = maxOrder.length > 0 ? (maxOrder[maxOrder.length - 1]?.displayOrder ?? 0) + 1 : 1
      db.insert(schema.pairs)
        .values({
          ...
          pipValuePerStandardLotCents: parsed.data.pipValuePerStandardLotCents,
          correlatedWith: parsed.data.correlatedWith ? JSON.stringify(parsed.data.correlatedWith) : null,
          active: 1,
          displayOrder,
  ...
  ipcMain.handle('pairs:update', (_e, raw: UpdatePairInput): IpcResponse<Pair> => {
    ...
    try {
      ...
      if (fields.pipValuePerStandardLotCents !== undefined) u.pipValuePerStandardLotCents = fields.pipValuePerStandardLotCents
      if (fields.active !== undefined) u.active = fields.active ? 1 : 0
      if (fields.displayOrder !== undefined) u.displayOrder = fields.displayOrder
      if ('correlatedWith' in fields) u.correlatedWith = fields.correlatedWith ? JSON.stringify(fields.correlatedWith) : null
