...
const PAIRS: Pair[] = [
  {
    id: 'p1', symbol: 'EURUSD', displayName: 'EUR/USD', assetClass: 'forex',
    pipDecimal: 4, pipValuePerStandardLotCents: 1000, correlatedWith: null,
    active: 1, displayOrder: 1, notes: null,
...
const SETUPS: Setup[] = [
  {
    id: 's1', name: 'ICT OB', category: 'ict', description: null,
...
describe('ImportTab', () => {
  ...
  it('renders broker options and the file-pick button', async () => {
    mockUseSessionStore.mockReturnValue({ selectedAccountId: 'acct-1' } as ReturnType<typeof useSessionStore>)
    // @ts-expect-error — partial mock
  ...
  it('shows the no-account warning when selectedAccountId is null', async () => {
    mockUseSessionStore.mockReturnValue({ selectedAccountId: null } as ReturnType<typeof useSessionStore>)
    // @ts-expect-error — partial mock
