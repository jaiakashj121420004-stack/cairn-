...
import {
  useLastTradeContextStore,
  getRecentContext,
} from '../../../src/stores/last-trade-context'

// ─── helpers ─────────────────────────────────────────────────────────────────

const ACCOUNT = 'acc-1'
const SAMPLE = { pairId: 'pair-eurusd', setupId: 'setup-fvg', mode: 'live' as const, accountId: ACCOUNT }
...
describe('useLastTradeContextStore', () => {
  ...
  it('setContext stores pair/setup/mode/accountId with a timestamp', () => {
    ...
    act(() => { result.current.setContext(SAMPLE) })
  ...
  it('is NOT persisted — remounting after setState still reflects store state, not disk', () => {
    ...
    act(() => { useLastTradeContextStore.setState({ context: null }) })
    rerender()
    expect(result.current.context).toBeNull()
  })
})
