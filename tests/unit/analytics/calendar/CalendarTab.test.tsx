...
const EMPTY_STATS = {
  totals: { tradeCount: 0, winCount: 0, lossCount: 0, winRateBps: 0, expectancyR: 0, totalR: 0, netPnlCents: 0, netPnlPctBps: 0, profitFactor: 0, avgWinCents: 0, avgLossCents: 0 },
  equityCurve: [],
  distribution: [],
...
vi.mock('../../../../src/stores/analytics-store', () => ({
  useAnalyticsStore: () => ({
    filter: { accountIds: 'all', dateFrom: null, dateTo: null, datePreset: 'all', mode: 'all', pairIds: [], setupIds: [], killzoneIds: [], cleanOnly: false },
  }),
}))

import { ipc } from '../../../../src/lib/ipc'
const mockPerformance = vi.mocked((ipc.analytics as { performance: typeof ipc.analytics.performance }).performance)
...
describe('CalendarTab', () => {
  ...
  it('shows trade count for a day', async () => {
    const date1 = currentMonthDate(1)
    const cells: DailyPnlCell[] = [
      { date: date1, pnlCents: 10000, tradeCount: 3, winCount: 2 },
    ]
    mockPerformance.mockResolvedValue(ok(makeStats(cells)))
    renderTab()
    await waitFor(() => {
  ...
  it('navigates to /trades?date=YYYY-MM-DD on day click', async () => {
    const date1 = currentMonthDate(1)
    const cells: DailyPnlCell[] = [
      { date: date1, pnlCents: 10000, tradeCount: 1, winCount: 1 },
    ]
    mockPerformance.mockResolvedValue(ok(makeStats(cells)))
    renderTab()
