...
describe('computeMaeMfe — hand-computed short fixture', () => {
  ...
  const input: MaeMfeInput = {
    ...
    direction:  'short',
    candles: [
      candle('1.27350', '1.27200'),
      candle('1.27280', '1.27100'),
    ],
  }
...
describe('computeMaeMfe — returns null (no guessing)', () => {
  ...
  it('malformed candle value → null (whole series untrusted)', () => {
    expect(
      computeMaeMfe({ ...base, candles: [candle('1.086', 'abc')] }),
    ).toBeNull()
  })

  it('empty-string price → null', () => {
    expect(
      computeMaeMfe({ ...base, slPrice: '', candles: [candle('1.086', '1.084')] }),
    ).toBeNull()
  })
})
    ...
    fc.assert(
      fc.property(
        ...
        (entryTicks, riskTicks, direction, otherRaw) => {
          ...
          const slTicks =
            direction === 'long' ? entryTicks - riskTicks : entryTicks + riskTicks

          const others = otherRaw.map((c) =>
            candle(ticksToStr(c.low + c.span), ticksToStr(c.low)),
          )
