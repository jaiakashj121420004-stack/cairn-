...
const RULES = [
  { key: 'no_sl_widening', label: 'No SL Widening', description: '', category: 'process', severity: 'blocking', isHardLock: false },
  { key: 'require_killzone', label: 'Require Killzone', description: '', category: 'timing', severity: 'blocking', isHardLock: false },
  { key: 'min_rr_ratio', label: 'Min RR Ratio', description: '', category: 'risk', severity: 'blocking', isHardLock: false },
]
...
function buildMockApi(detected: Array<{ ruleKey: string; detail: string }>) {
  return {
    rules: {
      ...
      override: vi.fn(), clearCooldown: vi.fn(), onTradeClosed: vi.fn(),
    },
    trades: {
      get: () => ok({ ruleViolations: [] }),
      close: vi.fn(), partialClose: vi.fn(), pickScreenshots: vi.fn(),
      addScreenshot: vi.fn(), listScreenshots: vi.fn(), deleteScreenshot: vi.fn(),
...
describe('CloseTradeModal — pre-tick from detected violations', () => {
  ...
  it('pre-ticks the detected rule and shows the detected badge', async () => {
    ...
    window.api = buildMockApi([
      { ruleKey: 'no_sl_widening', detail: 'Stop loss was moved from the plan (1079000) to 1078000.' },
    ])
    renderModal()
  ...
  it('leaves undetected rules unticked and unbadged', async () => {
    // @ts-expect-error — partial mock is sufficient for what the modal calls
    window.api = buildMockApi([
      { ruleKey: 'no_sl_widening', detail: 'Stop loss widened.' },
    ])
    renderModal()
    ...
    const kzCheckbox = screen.getByRole('checkbox', { name: /Require Killzone/i }) as HTMLInputElement
