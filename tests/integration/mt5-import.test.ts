...
function makeDb() {
  ...
  db.insert(schema.propFirms).values({
    id: PROP_FIRM_ID, name: 'Test Firm', defaultStepCount: 1,
    notes: null, createdAt: now, updatedAt: now, deletedAt: null,
  }).run()

  // Account
  db.insert(schema.accounts).values({
    id: ACCOUNT_ID, displayName: 'Demo', templateId: null,
    propFirmId: PROP_FIRM_ID, stepCount: 1, currentPhase: 1,
    accountSizeCents: 1_000_000, leverage: 100,
    ...
    consistencyRulePct: null, challengeCostCents: 10000,
  ...
  db.insert(schema.setups).values({
    id: SETUP_ID, name: 'ICT OB', category: 'ict',
    description: null, color: '#4CAF50', active: 1, displayOrder: 1,
    createdAt: now, updatedAt: now,
  }).run()

  // Pairs
  db.insert(schema.pairs).values({
    id: EURUSD_PAIR_ID, symbol: 'EURUSD', displayName: 'EUR/USD',
  ...
  db.insert(schema.pairs).values({
    id: GBPUSD_PAIR_ID, symbol: 'GBPUSD', displayName: 'GBP/USD',
    assetClass: 'forex', pipDecimal: 4,
    pipValuePerStandardLotCents: 1000, correlatedWith: null,
    active: 1, displayOrder: 2, notes: null, createdAt: now, updatedAt: now,
  }).run()
  db.insert(schema.pairs).values({
    id: USDJPY_PAIR_ID, symbol: 'USDJPY', displayName: 'USD/JPY',
    assetClass: 'forex', pipDecimal: 2,
    pipValuePerStandardLotCents: 909, correlatedWith: null,
    active: 1, displayOrder: 3, notes: null, createdAt: now, updatedAt: now,
  }).run()
...
describe('import:previewMt5', () => {
  it('returns 3 known candidates + 1 unresolved + 0 errors', () => {
    makeDb()
    const preview = unwrap(call<Mt5ImportPreview>('import:previewMt5', {
      html: FIXTURE_HTML,
...
describe('import:commitMt5 — happy path', () => {
  it('imports 3 trades and 1 partial, skips the XYZABC candidate', () => {
    ...
    const result = unwrap(call<Mt5CommitResult>('import:commitMt5', {
      html: FIXTURE_HTML,
      accountId: ACCOUNT_ID,
  ...
  it('stores external_ref on the imported trade row', () => {
    const { db } = makeDb()
    unwrap(call<Mt5CommitResult>('import:commitMt5', {
      html: FIXTURE_HTML,
      accountId: ACCOUNT_ID,
      symbolMap: { XYZABC: EURUSD_PAIR_ID },
      defaultSetupId: SETUP_ID,
    }))
    const rows = db.select({ externalRef: schema.trades.externalRef })
      .from(schema.trades).all()
  ...
  it('stores external_ref on the partial close row', () => {
    ...
    const partials = db.select({ externalRef: schema.tradePartials.externalRef })
      .from(schema.tradePartials).all()
  ...
  it('sets brokerSource to "mt5" on all imported trades', () => {
    ...
    const imported = db.select({ brokerSource: schema.trades.brokerSource })
      .from(schema.trades).all()
  ...
  it('EURUSD trade has correct encoded entry/exit prices', () => {
    ...
    const row = rows.find((_, i) => {
      const refRows = db.select({ externalRef: schema.trades.externalRef }).from(schema.trades).all()
...
describe('import:commitMt5 — idempotency', () => {
  it('re-importing the same HTML imports 0 new trades and reports the skipped count', () => {
    ...
    const second = unwrap(call<Mt5CommitResult>('import:commitMt5', {
      html: FIXTURE_HTML,
      accountId: ACCOUNT_ID,
      symbolMap: { XYZABC: EURUSD_PAIR_ID },
      defaultSetupId: SETUP_ID,
...
describe('import:commitMt5 — validation errors', () => {
  it('rejects commit when a symbol is still unresolved', () => {
    ...
    if (!res.ok) {
      ...
      expect((res.error.details as { unresolvedSymbols: string[] }).unresolvedSymbols).toContain('XYZABC')
    }
  })
