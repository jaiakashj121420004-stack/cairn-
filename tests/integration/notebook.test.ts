...
describe('notebook IPC CRUD', () => {
  it('creates, lists, gets, updates, pins and soft-deletes an entry', () => {
    ...
    const created = unwrap(
      call<NotebookEntry>('notebook:create', { title: 'Plan', content: '# Plan\n- a', template: 'trading_plan' }),
    )
    ...
    const updated = unwrap(
      call<NotebookEntry>('notebook:update', { id: created.id, title: 'Plan v2', content: 'changed' }),
    )
