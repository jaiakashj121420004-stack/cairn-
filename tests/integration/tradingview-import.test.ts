...
function makeDb() {
  ...
  for (const [id, symbol, pd] of [
    ...
    db.insert(schema.pairs).values({
      id, symbol, displayName: symbol, assetClass: 'forex',
      pipDecimal: pd, pipValuePerStandardLotCents: 1000,
      correlatedWith: null, active: 1, displayOrder: 1,
...
describe('import:previewTradingView', () => {
  it('returns 3 auto-resolved candidates for the simple fixture (EURUSD, GBPUSD, USDJPY)', () => {
    makeDb()
    const preview = unwrap(call<ImportPreview>('import:previewTradingView', {
      csv: SIMPLE_CSV, accountId: ACCOUNT_ID,
    }))
  ...
  it('partials fixture: both EURUSD and GBPJPY auto-resolve (GBPJPY is a migration-seeded pair)', () => {
    ...
    makeDb()
    const preview = unwrap(call<ImportPreview>('import:previewTradingView', {
      csv: PARTIALS_CSV, accountId: ACCOUNT_ID,
    }))
  ...
  it('XYZABC (not a migration-seeded pair) appears in unresolvedSymbols', () => {
    makeDb()
    const preview = unwrap(call<ImportPreview>('import:previewTradingView', {
      csv: XYZABC_CSV, accountId: ACCOUNT_ID,
    }))
  ...
  it('rejects an invalid accountId', () => {
    ...
    const res = call<ImportPreview>('import:previewTradingView', {
      csv: SIMPLE_CSV, accountId: 'not-a-uuid',
    })
...
describe('import:commitTradingView — happy path', () => {
  it('imports 3 trades from the simple fixture, 0 partials, 0 skipped', () => {
    makeDb()
    const result = unwrap(call<ImportCommitResult>('import:commitTradingView', {
      csv: SIMPLE_CSV, accountId: ACCOUNT_ID,
      symbolMap: {},
  ...
  it('stores tv_trade_N externalRefs on committed trades', () => {
    const { db } = makeDb()
    unwrap(call<ImportCommitResult>('import:commitTradingView', {
      csv: SIMPLE_CSV, accountId: ACCOUNT_ID,
      symbolMap: {},
      defaultSetupId: SETUP_ID,
    }))
    const refs = db.select({ externalRef: schema.trades.externalRef })
      .from(schema.trades).all()
      .map((r) => r.externalRef).filter(Boolean)
  ...
  it('stores USDJPY entry price with correct pipDecimal=2 encoding', () => {
    ...
    const trade = db.select({ entryPrice: schema.trades.entryPrice })
      .from(schema.trades)
...
describe('import:commitTradingView — idempotency', () => {
  it('second import of the same file returns imported=0 and skipped=3', () => {
    ...
    const second = unwrap(call<ImportCommitResult>('import:commitTradingView', {
      csv: SIMPLE_CSV, accountId: ACCOUNT_ID,
      symbolMap: {},
      defaultSetupId: SETUP_ID,
    }))
...
describe('import:commitTradingView — validation errors', () => {
  it('rejects commit when a symbol is unresolved (XYZABC not in migration seeds)', () => {
    ...
    if (!res.ok) {
      ...
      expect((res.error.details as { unresolvedSymbols: string[] }).unresolvedSymbols)
        .toContain('XYZABC')
    }
  })
