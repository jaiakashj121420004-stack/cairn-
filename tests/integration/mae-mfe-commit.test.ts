...
const OPTS = { accountId: ACCOUNT_ID, defaultSetupId: SETUP_ID, brokerSource: 'tradingview', nowMs: Date.UTC(2024, 0, 2) }
...
function readTrade(db: ReturnType<typeof makeDb>) {
  ...
  }).from(schema.trades).where(eq(schema.trades.externalRef, 'tv_trade_1')).get()
}

describe('committer — MAE/MFE storage', () => {
  it('stores mae_pips / mfe_pips (tenths) when a price series is present', () => {
    const db = makeDb()
    const res = commitCandidates(db, [candidate({ priceSeries: SERIES })], OPTS, PAIR_DETAIL, ACCOUNT)
  ...
  it('leaves them null when a series is present but there is no stop-loss', () => {
    const db = makeDb()
    commitCandidates(db, [candidate({ stopLoss: null, priceSeries: SERIES })], OPTS, PAIR_DETAIL, ACCOUNT)
    const trade = readTrade(db)
    expect(trade?.maePips).toBeNull()
    expect(trade?.mfePips).toBeNull()
  })
