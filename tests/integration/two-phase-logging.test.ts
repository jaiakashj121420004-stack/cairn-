...
function seed(db: ReturnType<typeof makeDb>) {
  ...
  db.insert(schema.accounts).values({
    id: ACCOUNT_ID, displayName: 'TestAccount', propFirmId: firm.id, stepCount: 1,
...
function createFastPathTrade(pairId: string, setupId: string): Trade {
  return unwrap<Trade>(call('trades:create', {
    accountId: ACCOUNT_ID, pairId, setupId,
...
describe('Phase 1 → minimal close', () => {
  it('closing a trade minimally adds it to the reflection queue (badge 0 → 1)', () => {
    ...
    const closed = unwrap<Trade>(call('trades:closeMinimal', {
      tradeId: trade.id, exitPrice: 109_200_0, exitTime: Date.now() + 60_000, exitReason: 'tp',
    }))
    ...
    const queue = unwrap<TradeListItem[]>(call('trades:listAwaitingReflection', { accountId: null }))
...
describe('Phase 2 — completing the deferred reflection', () => {
  it('completing Phase 2 clears the trade from the queue (badge 1 → 0) and persists honesty', () => {
    ...
    const { pairId, setupId } = seed(db)
    const trade = createFastPathTrade(pairId, setupId)
    unwrap<Trade>(call('trades:closeMinimal', {
      tradeId: trade.id, exitPrice: 109_200_0, exitTime: Date.now() + 60_000, exitReason: 'tp',
    }))
    expect(pendingCount()).toBe(1)

    const reflected = unwrap<Trade>(call('trades:completePhase2', {
      tradeId: trade.id,
  ...
  it('rejects a second Phase-2 completion (idempotency guard)', () => {
    ...
    unwrap<Trade>(call('trades:completePhase2', {
      tradeId: trade.id, followedPlanExactly: true, slMoved: false, enteredBeforeMss: false,
      rulesBroken: [], postCalmScore: 7,
    }))
  ...
  it('a rule-breaking reflection marks the trade not-clean and records a violation', () => {
    ...
    const trade = createFastPathTrade(pairId, setupId)
    unwrap<Trade>(call('trades:closeMinimal', {
      tradeId: trade.id, exitPrice: 108_900_0, exitTime: Date.now() + 60_000, exitReason: 'sl',
    }))
