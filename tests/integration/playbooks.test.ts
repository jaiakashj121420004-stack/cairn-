...
function makeDb() {
  ...
  db.insert(schema.setups).values({
    id: SETUP_ID, name: 'ICT FVG', category: 'ict',
  ...
  db.insert(schema.killzones).values({
    id: KZ_ID, name: 'London Open', startTimeUtc: '07:00', endTimeUtc: '10:00',
...
describe('playbooks:create', () => {
  it('creates a playbook and returns it with all fields', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', {
  ...
  it('rejects when name is empty', () => {
    makeDb()
    const res = call<Playbook>('playbooks:create', { accountId: ACCOUNT_ID, name: '', setupId: SETUP_ID })
...
describe('playbooks:list', () => {
  it('returns all non-deleted playbooks for the account, ordered by name', () => {
    ...
    unwrap(call('playbooks:create', { accountId: ACCOUNT_ID, name: 'Alpha FVG', setupId: SETUP_ID }))
  ...
  it('does not return soft-deleted playbooks', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', { accountId: ACCOUNT_ID, name: 'Deleted PB', setupId: SETUP_ID }))
...
describe('playbooks:update', () => {
  it('updates allowed fields and bumps version', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', {
      accountId: ACCOUNT_ID, name: 'Original Name', setupId: SETUP_ID,
    }))
    expect(pb.version).toBe(1)

    const updated = unwrap<Playbook>(call('playbooks:update', {
      id:           pb.id,
      name:         'Updated Name',
      defaultRiskPct: 200,
    }))
  ...
  it('can null out optional fields', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', {
      accountId: ACCOUNT_ID, name: 'With Pair', setupId: SETUP_ID, pairId: PAIR_ID,
    }))
...
describe('playbooks:delete', () => {
  it('soft-deletes a playbook (it is not returned by list)', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', {
      accountId: ACCOUNT_ID, name: 'To Delete', setupId: SETUP_ID,
    }))
    unwrap(call('playbooks:delete', { id: pb.id }))
  ...
  it('playbooks:get returns NOT_FOUND after soft-delete', () => {
    makeDb()
    const pb = unwrap<Playbook>(call('playbooks:create', {
      accountId: ACCOUNT_ID, name: 'To Delete Get', setupId: SETUP_ID,
    }))
    unwrap(call('playbooks:delete', { id: pb.id }))
