...
function SortIcon({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey; sortDir: SortDir }) {
  if (col !== sortKey) return <ChevronsUpDown className="h-3 w-3 text-text-muted" strokeWidth={1.5} />
  return sortDir === 'asc'
...
function StatusBadge({ status }: { status: TradeStatus }) {
  const map: Record<TradeStatus, { label: string; variant: 'success' | 'warning' | 'default' | 'neutral' }> = {
...
export function TradeLogPage() {
  ...
  const load = useCallback(() => {
    ...
    ipc.trades.list(filter).then((res) => {
      setLoading(false)
      if (res.ok) {
        setTrades(res.data)
      }
    })
  }, [selectedAccountId, filterMode, filterDirection, filterStatus, filterClean, filterDateFrom, filterDateTo])
  ...
  const hasFilters = filterPair || filterMode || filterDirection || filterStatus || filterClean || filterDateFrom || filterDateTo
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          {!loading && (
            <span className="text-caption text-text-muted">{sorted.length} trades</span>
          )}
        </div>
          ...
          {selectedIds.size === 0 && (
            <>
              ...
              >
                ...
                Filters
                {hasFilters && (
                  <span className="ml-1.5 h-1.5 w-1.5 rounded-full bg-accent-a" />
                )}
              </Button>
            </>
          )}
          ...
          <table className="w-full border-collapse text-body-sm">
            <thead className="sticky top-0 z-10 glass-strong" style={{ borderBottom: '1px solid var(--glass-border)' }}>
              <tr>
                ...
                <Th col="durationMinutes" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} right>
                  Duration
                </Th>
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-right font-mono">
                      {trade.pnlCents !== null ? (
                        <span
                          className={
                            trade.pnlCents >= 0 ? 'text-accent-a' : 'text-danger'
                          }
                        >
                    ...
                    <td className="px-3 py-2.5 text-right font-mono">
                      {trade.pnlR !== null ? (
                        <span
                          className={trade.pnlR >= 0 ? 'text-accent-a' : 'text-danger'}
                        >
