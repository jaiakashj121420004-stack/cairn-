    ...
    <div className="max-w-sm space-y-6">
      ...
      <div className="flex flex-col gap-2 border-t border-border pt-6">
        <div className="flex items-center justify-between gap-4">
          <label htmlFor="fast-path-toggle" className="text-caption font-medium text-text-secondary">
        ...
        <p className="text-caption text-text-muted leading-relaxed">
          {fastPath ? (
            <>
              <span className="text-text-secondary">On.</span> The New Trade panel asks only for what the
              rule engine needs in the moment — pair, direction, prices, one invalidation, one emotion tap —
              and defers the journal (honesty review, rules broken, MAE/MFE, notes) to a calm batch on the
              Review screen after the session.
            </>
          ) : (
            <>
              <span className="text-text-secondary">Off.</span> Every trade is logged in one pass: the full
              reflection form appears at entry and at close, with nothing deferred.
            </>
          )}
        </p>
        <p className="text-caption text-text-muted leading-relaxed">
