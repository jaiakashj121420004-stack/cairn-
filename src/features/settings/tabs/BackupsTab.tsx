...
export function BackupsTab() {
  ...
  async function pickFolder() {
    ...
    if (!r.ok) { toast(r.error.message, 'error'); return }
    ...
    <div className="max-w-xl space-y-8">
      ...
      <section className="space-y-3">
        ...
        <p className="text-caption text-text-muted">
          Where backup ZIP files are saved. Point this to a cloud-synced folder (Dropbox, iCloud, etc.) for automatic off-device backup.
        </p>
      ...
      <section className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-body-sm text-text-secondary">Also back up when closing the app</span>
        </div>
      ...
      <section className="space-y-2">
        ...
        <p className="text-caption text-text-muted">
          ...
          {settings.folder ? ' Saved to your configured folder.' : ' You will pick the destination folder.'}
        </p>
      ...
      <section className="space-y-2 rounded-xl border border-warning/30 bg-warning/5 p-4">
        ...
        <p className="text-caption text-text-muted">
          Select a Cairn backup ZIP. Your current data will be replaced and the app will restart. A safety copy of your current data is saved automatically first.
        </p>
      ...
      <Modal
        ...
        onClose={() => { setRestoreInfo(null); setRestoreConfirm('') }}
        ...
        {restoreInfo && (
          <div className="space-y-4">
            <div className="rounded-md border border-border bg-surface p-3 font-mono text-body-sm text-text-secondary space-y-1">
              <p>Version: <span className="text-text-primary">{restoreInfo.version}</span></p>
              <p>Created: <span className="text-text-primary">{formatTimestamp(restoreInfo.createdAt)}</span></p>
              <p>Trades: <span className="text-text-primary">{restoreInfo.tradeCount}</span></p>
              <p>Accounts: <span className="text-text-primary">{restoreInfo.accountCount}</span></p>
            </div>
            <p className="text-caption text-danger">
              This will permanently replace your current database. A pre-restore backup is saved automatically. The app will restart.
            </p>
            <div className="space-y-2">
              <p className="text-caption text-text-secondary">
                Type <span className="font-mono font-semibold text-text-primary">RESTORE</span> to confirm.
              </p>
              <Input
