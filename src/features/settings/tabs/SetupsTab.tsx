...
export function SetupsTab() {
  ...
  async function handleSave() {
    ...
    if (editing) {
      ...
      if (res.ok) { toast('Setup updated', 'success'); await load() }
      else toast(res.error.message, 'error')
    } else {
      const res = await ipc.setups.create(form)
      if (res.ok) { toast('Setup created', 'success'); await load() }
      else toast(res.error.message, 'error')
    }
    setSaving(false)
    setModalOpen(false)
        ...
        <table className="w-full text-body">
          <thead>
            <tr className="border-b border-border bg-surface">
              ...
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Category</th>
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Description</th>
            ...
            {visible.map((setup) => (
              <tr key={setup.id} className={setup.active === 0 ? 'opacity-50' : ''}>
                ...
                <td className="px-4 py-3 font-medium text-text-primary">
                  {setup.name}
                  {setup.active === 0 && <Badge variant="neutral" className="ml-2">archived</Badge>}
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <IconBtn onClick={() => void toggleArchive(setup)} title={setup.active === 1 ? 'Archive' : 'Restore'}>
                      {setup.active === 1
