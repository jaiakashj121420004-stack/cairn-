...
const ERROR_COPY: Record<string, string> = {
  VALIDATION_ERROR:    'The file could not be validated. Make sure you selected the correct export format.',
...
export function ImportTab() {
  ...
  useEffect(() => {
    void Promise.all([
      ipc.pairs.list().then((r) => { if (r.ok) setPairs(r.data) }),
      ipc.setups.list().then((r) => { if (r.ok) setSetups(r.data) }),
    ])
  }, [])
  ...
  async function handleCommit() {
    ...
    setStep('done')
    toast(
      `${res.data.imported} trade${res.data.imported === 1 ? '' : 's'} imported.`,
      'success',
    )
  }
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-0.5 text-caption text-text-muted">
            Import closed trades from a broker statement. Duplicate trades are automatically skipped.
          </p>
        </div>
      ...
      {step === 'preview' && preview && (
        <section className="space-y-5">
                ...
                <tbody>
                  {preview.candidates.map((c) => (
                    <tr key={c.externalRef} className="border-b border-border/40 last:border-0">
                      ...
                      >
                        {parseFloat(c.pnlAmount) >= 0 ? '+' : ''}{c.pnlAmount}
                      </td>
          ...
          {preview.parseErrors.length > 0 && (
            <ParseErrorList errors={preview.parseErrors} />
          )}
