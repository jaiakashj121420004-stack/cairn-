...
export function KillzonesTab() {
  ...
  async function handleSave() {
    ...
    if (editing) {
      ...
      if (res.ok) { toast('Killzone updated', 'success'); await load() }
      else toast(res.error.message, 'error')
    } else {
      const res = await ipc.killzones.create(form)
      if (res.ok) { toast('Killzone created', 'success'); await load() }
      else toast(res.error.message, 'error')
    }
    setSaving(false)
    setModalOpen(false)
        ...
        <table className="w-full text-body">
          <thead>
            <tr className="border-b border-border bg-surface">
              ...
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Start (UTC)</th>
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">End (UTC)</th>
            ...
            {visible.map((kz) => (
              <tr key={kz.id} className={kz.active === 0 ? 'opacity-50' : ''}>
                ...
                <td className="px-4 py-3 font-medium text-text-primary">
                  {kz.name}
                  {kz.active === 0 && <Badge variant="neutral" className="ml-2">archived</Badge>}
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <IconBtn onClick={() => void toggleArchive(kz)} title={kz.active === 1 ? 'Archive' : 'Restore'}>
                      {kz.active === 1
