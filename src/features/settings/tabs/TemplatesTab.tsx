...
export function TemplatesTab() {
  ...
  async function handleCreate() {
    ...
    if (res.ok) { toast('Template created', 'success'); await load(); setCreateOpen(false) }
  ...
  async function handleEditSave() {
    ...
    if (res.ok) { toast('Template updated', 'success'); await load(); setEditTarget(null) }
    else toast(res.error.message, 'error')
    setSaving(false)
  }

  async function toggleArchive(tmpl: AccountTemplate) {
    const res = await ipc.accountTemplates.update({ id: tmpl.id, isArchived: tmpl.isArchived !== 1 })
  ...
  return (
    <div className="space-y-4">
      ...
      {firms.length === 0 && (
        <p className="text-caption text-text-muted">Add a prop firm first before creating templates.</p>
      )}
        ...
        <table className="w-full text-body">
          <thead>
            <tr className="border-b border-border bg-surface">
              ...
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Firm</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Size</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Daily DD</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Total DD</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Target</th>
            ...
            {visible.map((tmpl) => (
              <tr key={tmpl.id} className={tmpl.isArchived === 1 ? 'opacity-50' : ''}>
                <td className="px-4 py-3 font-medium text-text-primary">
                  {tmpl.name}
                  {tmpl.isArchived === 1 && <Badge variant="neutral" className="ml-2">archived</Badge>}
                </td>
                <td className="px-4 py-3 text-text-muted">{firmMap.get(tmpl.propFirmId) ?? '—'}</td>
                <td className="px-4 py-3 text-right font-mono text-text-secondary">{fmtSize(tmpl.accountSizeCents)}</td>
                <td className="px-4 py-3 text-right font-mono text-text-secondary">{fmtBps(tmpl.dailyDrawdownValue)}</td>
                <td className="px-4 py-3 text-right font-mono text-text-secondary">{fmtBps(tmpl.totalDrawdownValue)}</td>
                <td className="px-4 py-3 text-right font-mono text-text-secondary">{fmtBps(tmpl.profitTargetPhase1Pct)}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <IconBtn onClick={() => openEdit(tmpl)} title="Edit">
                      <Pencil className="h-3.5 w-3.5" strokeWidth={1.5} />
                    </IconBtn>
                    <IconBtn onClick={() => void toggleArchive(tmpl)} title={tmpl.isArchived === 1 ? 'Restore' : 'Archive'}>
                      {tmpl.isArchived === 1
      ...
      {/* Create modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Add template" maxWidth="540px">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select
              ...
              onChange={(v) => setForm({ ...form, dailyDrawdownType: v as CreateAccountTemplateInput['dailyDrawdownType'] })}
            />
            <Select
              label="DD basis"
              options={DD_BASIS_OPTIONS}
              value={form.drawdownBasis}
              onChange={(v) => setForm({ ...form, drawdownBasis: v as CreateAccountTemplateInput['drawdownBasis'] })}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Input
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button onClick={() => void handleCreate()} loading={saving}>Create template</Button>
          </div>
        </div>
      </Modal>
