...
export function PlaybooksTab() {
  ...
  async function handleSave() {
    ...
    if (!form.setupId)      { toast('Setup is required', 'error'); return }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-caption text-text-muted mt-0.5">
            Saved trade templates. One tap pre-fills the New Trade panel with your
            pair, setup, killzone, default risk %, and invalidation chip.
          </p>
        </div>
      ...
      {playbooks.length === 0 ? (
        ...
        <div className="space-y-2">
          {playbooks.map((pb) => (
            <div
              key={pb.id}
              className="glass rounded-[12px] p-4 flex items-start gap-4"
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-caption text-text-muted">
                  ...
                  {pb.defaultInvalidationChip && (
                    <>
                      <span>·</span>
                      <span className="italic">{chipName(pb.defaultInvalidationChip) ?? pb.defaultInvalidationChip}</span>
                    </>
                  )}
                </div>
