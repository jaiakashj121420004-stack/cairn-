...
export function PairsTab() {
  ...
  function openEdit(pair: Pair) {
    ...
    setForm({
      ...
      ...(pair.correlatedWith ? { correlatedWith: JSON.parse(pair.correlatedWith) as string[] } : {}),
  ...
  async function handleSave() {
    ...
    if (editing) {
      ...
      if (res.ok) { toast('Pair updated', 'success'); await load() }
      else toast(res.error.message, 'error')
    } else {
      const res = await ipc.pairs.create(form)
      if (res.ok) { toast('Pair created', 'success'); await load() }
      else toast(res.error.message, 'error')
    }
    setSaving(false)
    setModalOpen(false)
        ...
        <table className="w-full text-body">
          <thead>
            <tr className="border-b border-border bg-surface">
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Symbol</th>
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Display name</th>
              <th className="px-4 py-2.5 text-left text-caption font-medium text-text-muted">Class</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Pip decimal</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted">Pip value</th>
              <th className="px-4 py-2.5 text-right text-caption font-medium text-text-muted w-32">Actions</th>
            </tr>
          </thead>
            ...
            {visible.map((pair) => (
              <tr key={pair.id} className={pair.active === 0 ? 'opacity-50' : ''}>
                <td className="px-4 py-3 font-mono font-semibold text-text-primary">
                  {pair.symbol}
                  {pair.active === 0 && <Badge variant="neutral" className="ml-2">archived</Badge>}
                </td>
                <td className="px-4 py-3 text-text-secondary">{pair.displayName}</td>
                <td className="px-4 py-3 text-text-muted">{pair.assetClass}</td>
                <td className="px-4 py-3 text-right font-mono text-text-secondary">{pair.pipDecimal}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <IconBtn onClick={() => void toggleArchive(pair)} title={pair.active === 1 ? 'Archive' : 'Restore'}>
                      {pair.active === 1
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              onChange={(e) => setForm({ ...form, pipValuePerStandardLotCents: Number(e.target.value) })}
