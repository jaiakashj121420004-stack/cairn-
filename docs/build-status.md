...
### 2026-07-29 — Root-caused and fixed the CI/Release/security failures on commit `04895b7` (commit `16684d4`, NOT yet pushed)

Akash pushed `04895b7` (the corrected `brace-expansion@^5.0.5` version-range override, replacing the rejected 3-level chain selector) and retagged `v0.2.1`, but **all three workflows — CI #12, Release #7, security #13 — still failed.** Root-caused from the actual Actions job logs (run `30416558451`), not guessed:

**Root cause:** every job across all three workflows opens with `pnpm install --frozen-lockfile`. All 5 CI jobs failed in 17–59s (too fast to be a real build/test failure — a tell that they all died at the same first step), and the Dependency Audit job's log confirmed it directly: `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH — The current "overrides" configuration doesn't match the value found in the lockfile`. `pnpm-lock.yaml` had last been regenerated in `e064eb7`, against the **first, blanket** brace-expansion override — before it was narrowed twice more (`1da1787`'s rejected 3-level selector, then `04895b7`'s working version-range selector). Neither of those two later edits ever got a matching lockfile regen, so `--frozen-lockfile` rejected the mismatch on every job, every workflow, immediately. `security.yml`'s `dep-audit` job runs the identical `pnpm install --frozen-lockfile` step, which is why it started failing too even though nothing in this saga had touched that file — closes the "why did security.yml start failing" question from last session.

**Fix:** regenerated `pnpm-lock.yaml` for real. The sandbox can't run `pnpm install` directly inside the mounted repo dir — two independent blockers, both confirmed by evidence, not assumed: (1) the mount disallows `unlink()` on files it doesn't like, which crashes pnpm's store hardlink-check partway through install (same family of quirk as the `.git/*.lock` issue worked around all session via `mv`-as-rename); (2) even in an isolated `/tmp` copy of just the workspace manifests, one dependency (`electron`'s `node-gyp`, fetched from `codeload.github.com` as a git tarball) hits a `403 Forbidden` — this sandbox's egress just can't reach it, a real environment limit, not something to route around. Given that, the actual fix path was: run `pnpm install --no-frozen-lockfile` directly in the mounted dir anyway — it turns out pnpm computes and writes the full corrected lockfile (overrides header + re-resolved dependency graph) *before* it reaches the store-check step that crashes, so the crash happens strictly after the file we need is already written correctly. Verified this wasn't a lucky/partial write by diffing the result against HEAD and re-running `pnpm install --frozen-lockfile` against it in place — that now passes the config-mismatch check cleanly (it proceeds to the same later sandbox-only EPERM, confirming the lockfile itself is no longer the problem).

**Diff is minimal and exactly right:** overrides header `brace-expansion@<5.0.8` → `brace-expansion@^5.0.5`; `minimatch@10.2.5` (the actually-vulnerable consumer, via `@fastify/otel`) keeps `brace-expansion@5.0.8` (patched); `minimatch@3.1.5`, `@5.1.9`, `@9.0.9` (ESLint's and others' consumers) revert from the incorrectly-forced `brace-expansion@5.0.8` back to their real `1.1.16`/`2.1.3` lines. This is the full, correct reversal of the ESLint-breaking blanket-override bug — the previously-diagnosed `3.1.5` case plus two more latent breaks (`5.1.9`, `9.0.9`) that hadn't been individually called out before.

**Git state:** committed directly as `16684d4` on top of `04895b7`. **Akash pushed it** (`04895b7..aaeb2fb`, along with the docs commit `aaeb2fb`) — confirmed via `git push` output. The config-mismatch is confirmed gone (see addendum below): every job now gets **past** `pnpm install --frozen-lockfile` for the first time in this saga.

**Lesson for next time a `pnpm.overrides` edit ships:** the lockfile's `overrides:` header is a checksum-like fingerprint of package.json's override block — `--frozen-lockfile` (which every CI job uses) fails hard and immediately the moment they diverge, before any real typecheck/test/build work even starts. Any future override edit needs `pnpm install` (regenerating the lockfile) as part of the *same* commit, not a follow-up — this is the second time in this saga a forgotten lockfile regen caused a full-pipeline failure.

**Addendum, same day — the lockfile fix worked; it unblocked 3 more real, distinct, previously-hidden failures (CI run `30470978607` / security run `30470978652`, commit `aaeb2fb`):** these jobs never even got far enough to hit them before, because `--frozen-lockfile` killed every job in the first ~30s. Now that install succeeds, Build, License Check, and Unit Tests all pass cleanly (confirms the pnl-calculator fix from the day before is solid on a real CI runner). Three real issues surfaced, each confirmed from the actual job log:

1. **Typecheck & Lint → Format check (Prettier) failed** on 6 files: `docs/cairn-app-guide.html`, `docs/connect-mt5-and-ctrader.html`, and the 4 `legal/*.html` drafts (all committed earlier this session, never run through Prettier). Fixed by adding them to `.prettierignore` (`fix(ci)` `3caf057`) rather than reformatting — these are hand-crafted, single-file documents (a hash-routed JS marketing site, a localStorage-backed interactive checklist, long-form legal text) with zero test coverage; a Prettier HTML reflow risks disturbing embedded `<script>` behavior or copy-paste-sensitive legal text for no real benefit. Matches the existing precedent for excluding `cairn-prompts-checklist.html`. Verified against `prettier@3.8.3` — the version actually pinned in `pnpm-lock.yaml` (checked via `grep`), not the `^3.4.2` floor in package.json; a first pass using 3.4.2 gave a spurious extra warning on `globals.css` that disappeared entirely under the real pinned version.
2. **Smoke E2E still failed** — the `1da1787` fix (`pnpm exec playwright install ...`) was itself incomplete: `@playwright/test` is a devDependency of `apps/desktop` only, not the repo root, so a bare `pnpm exec` run from the root can't find the binary (`ERR_PNPM_RECURSIVE_EXEC_FIRST_FAIL`). Fixed by scoping with `--filter @cairn/desktop`, matching how `build`/`test:e2e` are already invoked elsewhere in the same file (`fix(ci)` `3caf057`).
3. **security.yml's Trivy filesystem + image scan jobs both failed identically** with `Unable to resolve action aquasecurity/trivy-action@0.28.0, unable to find version 0.28.0`. Checked directly against the action's own GitHub tags page — no `0.28.0` or `v0.28.0` tag has ever existed there; this pin was broken from day one, unrelated to anything else in this saga. Bumped all 4 references to `v0.36.0`, the current latest tag (`fix(security)` `20a4636`).

**Not touched, deliberately:** Dependency Audit's react-router finding (2 high: `GHSA-chx6-hx7r-mcp5`, `GHSA-qwww-vcr4-c8h2`) is the same pre-existing, previously-documented, deliberately-deferred gap from the 2026-07-28 `fix(deps) 02c9cf1` entry — not new, not part of this fix.

**Not yet re-verified green** as of this writing — `3caf057` and `20a4636` are committed on top of `aaeb2fb` (which Akash already pushed). These two are **not yet pushed**. Next step: push, then re-check the Actions tab.

**Decision, same day — asked Akash how to handle the react-router finding; the real fix turned out to be much bigger than a version bump.** Presented 3 options (leave deferred / add a tracked audit exception / do the real migration). Akash chose the real migration — but investigating what that actually requires surfaced a much larger scope than "bump a version number": confirmed directly against the npm registry (`registry.npmjs.org/react-router/8.3.0`), react-router v8 requires `engines.node >=22.22.0` and `peerDependencies react/react-dom >=19.2.7`, and its release notes add a Vite ≥7 baseline. This repo is pinned to Node 20 (`.nvmrc`, every CI job) and React 18.3.1, with Vite 5.4.0 (desktop) / 6.0.5 (web). So the "real fix" is actually a coordinated Node + React + Vite + router major-version bump across the whole frontend toolchain, touching ~28 files that import `react-router-dom` (full list gathered) plus the now-fully-removed `react-router-dom` package itself (v8 deletes it; everything moves to `react-router`, with `RouterProvider`/`HydratedRouter` specifically in `react-router/dom`).

Given that real scope, Akash's final call: **defer the actual work to its own dedicated session** (not bundled into tonight's CI-fix work), but do it for real when that session runs — not a suppression. Wrote the full binding plan, discovered file list, breaking-change checklists (React 19's removed `defaultProps`/legacy refs, React Router v8's future-flags-now-default behavior changes), sandbox lockfile-regen gotchas, and a mandatory UI/UX + feature manual-QA checklist (every route in both apps, broker-connect Settings screens, the Dodo/Stripe/Razorpay billing-return redirect flow) into **`docs/react-router-v8-migration-plan.md`** (new file, referenced from CLAUDE.md §4 and §17.6). That file is the starting point for the next session on this — read it in full before touching any dependency.

### 2026-07-28 — Live-capture $0 P&L bug fix + pnl-calculator precision guard + marketing/broker-guide docs; 4 commits made, NOT yet pushed

Found via real live QA: a real MT5 trade with $41.50 broker profit showed **$0.00** on Cairn's Dashboard/Trade Log/Analytics (pnlR was correct — R is price-ratio-only, independent of lot size/pip value — so this was invisible until dollar figures were checked directly).

**Root cause + fix (`fix(broker)`, commit `4cf406f`):** `accumulator.ts`'s `toCandidate()` stubs `pnlAmount: '0'` for live `BrokerEvent`s (no broker-settled P&L is available on the wire — no commission/swap visibility). `committer.ts`'s `buildCandidateRows()` trusted that stub at face value for **every** row, live or statement. Added `priceDerivedPnlCents()` in `committer.ts`, reusing the existing tested `calculatePnl()` formula from `pnl-calculator.ts` (no forked money-math), gated strictly on `options.settledImport === false` (live ingest only) — statement imports are byte-for-byte unchanged, and the existing `STATEMENT_MONETARY_FIELDS` anti-clobber protection on reconciliation is untouched. Fixes `grossPnlCents` AND each partial's `pnlCents`. Since Dashboard/Analytics all derive from `trades.pnlCents`, fixing the source fixes Net P&L, Today's P&L, Profit Factor, Max Drawdown, equity curve, etc. — no separate dashboard code needed changing. Updated `broker-ingest.test.ts`: one assertion that encoded the old $0 behavior as expected is corrected (`pnlCents: 0` → `2_000`), plus 3 new hand-verified assertions (trade pnlCents/pnlR, partial pnlCents).

**Note on the trade that was already captured live before this fix shipped:** it will NOT retroactively correct itself — it's sitting in the DB with the old wrong `pnlCents`. Needs either a fresh live trade to prove the fix, or a one-off backfill script recomputing existing live (`imported_at IS NULL`) rows from their stored prices. Not yet built; ask if wanted.

**Separate, pre-existing bug found while re-running gates (`fix(pnl)`, commit `6e43750`):** `tests/unit/pnl-calculator.test.ts`'s property test (`direction flip negates pnl...`) failed on an unrelated, pre-existing precision bug — nothing to do with the broker fix above. `pnl-calculator.ts`'s `roundToInt()` does `Decimal.toDecimalPlaces(0,...).toNumber()`; decimal.js is exact to 20 significant figures, but the final `.toNumber()` is only exact up to `Number.MAX_SAFE_INTEGER` (2^53−1). `pnlPctBps` amplifies `pnlCents` by `10_000/accountSizeCents`, so the test's synthetic 1-cent account size combined with extreme price/lot/pip inputs silently overflowed that ceiling (JS added the two corrupted doubles and got `10048` instead of the true `10000`). Fixed by making `roundToInt()` round-trip its result through a fresh `Decimal` and **throw** instead of silently returning a corrupted integer if it doesn't match exactly — real trades never come close to this bound, so the throw is pure defense-in-depth (fail loud on corrupt inputs, never silently write wrong money — CLAUDE.md §2.5). Also raised the test's `arbAccount` floor from 1 cent to a still-permissive-but-realistic $100 (10,000 cents), since a sub-dollar account was the specific unrealistic input that tripped the ceiling — verified sufficient via an independent Python re-implementation of the exact rounding logic across all 64 arbitrary corner combinations plus 200,000 randomized extreme-biased cases (zero overflows, ~180× safety margin at the worst case).

**Verification status — IMPORTANT GAP:** Akash ran the 5 desktop gates on host and hit the pnl-calculator failure above; after both fixes, he re-ran `pnpm build` (PASS, 8.42s) and `pnpm --filter @cairn/desktop run dist:local` (produced `Cairn Setup 0.2.0.exe`, unsigned as expected — no cert configured), and `test:e2e` passed (1/1) — **but against the OLD build** (from before the `fix(pnl)` commit; asset hashes were identical across both screenshots, confirming no rebuild happened between). **`pnpm typecheck` and `pnpm test:unit` have NOT been re-run since the `fix(pnl)` change landed.** I verified the fix is correct independently (exact-arithmetic Python simulation, not the actual TS toolchain — this sandbox can't run this repo's `tsc`/`vitest`, `node_modules` isn't functional here). Next session: if anything looks off in `pnl-calculator.test.ts`, re-run `pnpm typecheck && pnpm test:unit` fresh as the first move.

**New docs, committed (`docs:`, commit `880eb4b`):** `docs/cairn-app-guide.html` — single-file, JS hash-routed marketing + full features site (every screen, Free vs Pro split throughout: onboarding, dashboard, accounts/phases, rule engine, killzones, analytics, notebook, security, broker connect, pricing), themed in real Nvexis v3.0 Day/Night tokens. `docs/connect-mt5-and-ctrader.md` — beginner step-by-step MT5+cTrader connection guide (written from a real live connection walkthrough, includes real snags hit: accidentally-deleted EA, MT5-login-vs-bridge-connection confusion). `docs/connect-mt5-and-ctrader.html` — interactive checklist version of the same guide with localStorage-persisted progress.

**Also committed this session (pre-existing uncommitted work from an earlier session, not new today) (`docs(legal):`, commit `f1621c0`):** `legal/{privacy-policy,terms-of-service,refund-and-cancellation-policy,subprocessors}.html` — drafts, still need `[bracketed]` placeholders filled + lawyer review before public launch.

**Git state:** all 4 commits made directly on `feat/rules-engine` (`4cf406f`, `6e43750`, `880eb4b`, `f1621c0`), working tree clean. Branch is **33 commits ahead of `origin/feat/rules-engine`** (the prior 29-commit gap from the 2026-07-12 launch-readiness batch was never pushed either) — **none of this has been pushed to GitHub yet.** No push credentials are available from Claude's sandbox (no SSH key, no credential helper, no `gh` auth) — Akash needs to `git push origin feat/rules-engine` himself. Repo: `https://github.com/jaiakashj121420004-stack/cairn-` (note trailing dash in the repo name). Release pipeline (`.github/workflows/release.yml`) is already wired: pushing a `v*.*.*` tag triggers a cross-platform (win/mac/linux) build and opens a **draft** GitHub Release with checksums — signing is inert until certs are configured (unsigned build + warning, not a failure), publishing the draft is a manual step. Existing tags stop at `v0.2.0` (already on GitHub, pointing at the pre-this-session tip of `feat/rules-engine`) — the next release needs a new tag (e.g. `v0.2.1`).

**NEXT:** (1) Akash re-runs `pnpm typecheck && pnpm test:unit` to close the verification gap above. (2) `git push origin feat/rules-engine`. (3) Bump version (root + `apps/desktop` `package.json`) if desired, commit, tag `vX.Y.Z`, push the tag → CI cuts a draft release automatically. (4) Review + publish the draft on GitHub. (5) Optionally build the live-trade backfill script for the one already-captured $0 GBPUSD trade. (6) cTrader still blocked on Spotware KYC approval (external dependency, no action pending).

**Addendum, same day — found + fixed a real, pre-existing CI pipeline break (`ci:` commit `a1c0d4d`):** Pushed `feat/rules-engine` and tag `v0.2.1` per the plan above; both the `Release` workflow run AND the `CI` workflow run failed. Root cause (confirmed via the Actions run logs): `pnpm/action-setup@v4` in `ci.yml` (all 6 jobs), `release.yml`, and `backup-verify.yml` used `version: latest`, which now resolves to pnpm 11.17.0 — pnpm dropped Node 20 support in that line (needs the `node:sqlite` built-in, Node ≥22.13), while every job still sets up Node 20 via `.nvmrc`. Every one of those jobs has therefore been crashing at the `pnpm install` step with `ERR_UNKNOWN_BUILTIN_MODULE` — **CI has been red since at least commit `efef4cc`, and neither the v0.2.0 nor v0.2.1 tag push ever produced a real release build.** (The v0.2.0 GitHub release that already exists was published manually, not by this pipeline — confirmed no draft v0.2.1 release exists.) `security.yml` was the one workflow that was never on `latest` (already pinned to `10.33.0`) and has been passing the whole time — that's what gave away the fix. Pinned `ci.yml`/`release.yml`/`backup-verify.yml` to the same `10.33.0`. **Not yet pushed** (commit `a1c0d4d`, on top of the 4 commits above — 5 total unpushed as of this writing). **Still TODO next session:** confirm this actually fixes CI/Release once pushed — delete + recreate the `v0.2.1` tag (it pointed at the pre-fix commit) and push again, then watch the Actions tab for a green run and a real draft release this time.

**Second addendum, same day — the pnpm pin fix worked; it uncovered 3 more real, independent bugs underneath it (all fixed except one deliberate exception):** Akash pushed `a1c0d4d` + retagged `v0.2.1`. The pnpm crash is gone — **Unit Tests now pass on a clean CI Windows runner**, which is the independent, fresh-environment confirmation that was missing for the `fix(pnl)` precision guard (closes that verification gap for real, not just via my Python simulation). But three more issues surfaced, all diagnosed from the actual Actions run logs (`run 30383099804`), not guessed:

1. **`fix(desktop)` `0f824a2` — Modal.tsx casing bug (Typecheck & Lint failed, TS1261).** `git ls-files` showed the tracked path as `modal.tsx` while the on-disk file and all 4 import sites used `Modal.tsx` — a case-only rename that Windows/macOS git silently tolerates (case-insensitive filesystems don't notice) but that `tsc`'s cross-platform casing check catches on a fresh clone. Fixed via a two-step git rename (through a temp name — a same-name-different-case `git mv` is a no-op on this filesystem otherwise). Repo-wide scan confirmed no other such mismatches exist.
2. **`fix(ci)` `3682a5c` — wrong artifact path (Smoke E2E failed, cascading).** `pnpm build` writes to `apps/desktop/out/` (electron-vite runs from that cwd), but `ci.yml`'s Build job uploaded from repo-root `out/` (never existed → uploaded nothing) and Smoke E2E downloaded to the same wrong path. Confirmed against `tests/e2e/smoke.spec.ts`'s own `MAIN_JS` resolution (`path.resolve(__dirname, '..', '..', 'out', 'main', 'main.js')`, relative to `apps/desktop/tests/e2e`) before fixing both paths to `apps/desktop/out/`.
3. **`fix(deps)` `02c9cf1` — Dependency Audit failed, 11 high findings across 8 packages.** Two of the 3 existing `pnpm.overrides` (shell-quote, protobufjs, brace-expansion) had gone stale — newer advisories widened the vulnerable range past what they pinned to. Updated those 3 and added 4 new overrides (adm-zip, `@opentelemetry/propagator-jaeger`, fast-uri, find-my-way). **Deliberately NOT fixed:** react-router's 2 advisories need `react-router-dom` >=8.3.0, a major bump from the current 7.17.0 used across 26 files (`apps/desktop` + `apps/web`) — forcing that blind, untested, in a financial app, is a worse call than leaving Dependency Audit red on 2 known findings until someone actually does the migration + a navigation smoke-test. This is a tracked, open follow-up, not an oversight.

**Lockfile caveat — action required before pushing:** `pnpm-lock.yaml` was NOT regenerated for the override change. The sandbox's egress blocks a git-tarball transitive dep mid-resolution (`ERR_PNPM_FETCH_403` on `codeload.github.com/electron/node-gyp`), so `pnpm install --lockfile-only` cannot complete here even in an isolated scratch copy (confirmed — tried it). **Akash must run `pnpm install` locally and commit the updated `pnpm-lock.yaml` before pushing**, or CI's `--frozen-lockfile` step will fail on the mismatch (a 4th, entirely avoidable failure). Full command sequence given in chat.

**Meta-lesson:** the pnpm-pin fix was necessary but not sufficient — CI had been red long enough (since `efef4cc`) that real, unrelated bugs (the casing bug, the artifact path, the stale overrides) had been quietly accumulating underneath a gate that never actually ran. Worth remembering next time a CI pipeline has been silently red for a while: fixing the surface error often just reveals the next layer, budget for that.

**Third addendum, same day — the blanket brace-expansion override broke ESLint itself, plus a pnpm-10 regression (`fix(deps)` `1da1787`):** Akash regenerated the lockfile locally (`pnpm install` worked fine on his machine — the sandbox's egress block on a git-tarball dep is sandbox-only) and pushed. CI run `30387468799` made real progress — Build, License Check, and Unit Tests all passed, `electron-out` artifact uploaded correctly (2.01 MB, confirming the artifact-path fix), and Dependency Audit is down to exactly the 2 expected react-router findings (5 moderate/2 high, down from 7/11 — confirms the other 7 overrides are correct). But two new things broke, both root-caused from the actual job logs:

1. **The blanket `brace-expansion@<5.0.8` override broke ESLint itself.** It applied to *every* resolution of brace-expansion in the tree, including inside `minimatch@3.1.5` (an `@eslint/eslintrc` dependency, used for `overrides[].files` matching), which expects brace-expansion's old 1.x/2.x callable-function API — brace-expansion 5.x isn't shaped that way, so every `pnpm lint` crashed with `TypeError: expand is not a function`. This is exactly what crashed Akash's local pre-commit hook earlier too, not a separate issue. Fixed by scoping the override to the actual vulnerable chain from the original audit path — `"@fastify/otel>minimatch>brace-expansion": ">=5.0.8"` (pnpm supports `parent>child` chain keys) — which patches only that subtree and leaves `@eslint/eslintrc`'s own minimatch edge alone.
2. **Smoke E2E's `pnpm playwright install --with-deps chromium` failed:** `'playwright' is not recognized as an internal or external command`. Pinning pnpm from `latest` (11.17.0) down to `10.33.0` (the CI-pin fix) lost a newer-pnpm convenience feature that resolves an unrecognized bare command straight to a `node_modules/.bin` binary. Fixed with `pnpm exec playwright install ...` — the version-stable way to invoke a local binary on any pnpm ≥9. Grepped every workflow for the same `pnpm <bare-command>` pattern; this was the only non-package.json-script instance.

**Lockfile needs regenerating again** (override key changed) — same sandbox limitation, Akash runs `pnpm install` locally, commits, pushes. **Not yet re-verified green** as of this writing.
