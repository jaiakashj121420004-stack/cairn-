...
**Not touched, deliberately:** Dependency Audit's react-router finding (2 high: `GHSA-chx6-hx7r-mcp5`, `GHSA-qwww-vcr4-c8h2`, need `react-router-dom` ≥8.3.0, currently 7.17.0 across 26 files) is the same pre-existing, previously-documented, deliberately-deferred gap from the 2026-07-28 `fix(deps) 02c9cf1` entry — not new, not part of this fix. Still an open decision: do the real major-version migration, or add a documented, tracked audit exception for these two specific advisories so the gate can go green without a blind untested bump. Ask Akash before picking one.

**Not yet re-verified green** as of this writing — `3caf057` and `20a4636` are committed on top of `aaeb2fb` but **not yet pushed**. Next step: push, then re-check the Actions tab.
