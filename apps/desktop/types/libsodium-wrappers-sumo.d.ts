...
// pointed at by a `paths` entry in tsconfig.json / tsconfig.node.json so the checker
// uses this narrow surface instead. It does NOT affect runtime/bundler resolution
// (Vite, electron-vite, and vitest load the real package from node_modules).
//
// Keep this in sync with `SodiumApi` in electron/services/crypto/sodium.ts — both
// describe the exact slice of libsodium the app uses.
