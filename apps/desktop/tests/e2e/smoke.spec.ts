...
async function openSelect(page: import('@playwright/test').Page, placeholder: string) {
  await page.locator('button[aria-haspopup="listbox"]').filter({ hasText: placeholder }).click()
}
...
test('smoke: onboard → session bias → place trade → close trade → analytics', async () => {
  ...
  try {
    ...
    await expect(
      page.locator('p', { hasText: 'Trades' }).locator('xpath=following-sibling::p'),
    ).toHaveText('1')
    ...
    const pairTrigger = page
      .locator('button[aria-haspopup="listbox"]')
      .filter({ hasText: /Select pair…|EURUSD/ })
