...
describe('detectTpNarrowed', () => {
  ...
  it('short: actual target below the plan (extended) is not narrowing', () => {
    expect(detectTpNarrowed({ direction: 'short', plannedTp: 107800, actualTp: 107700 })).toBe(
      false,
    )
  })
})
...
describe('detectOutsideKillzone', () => {
  ...
  it('no killzones configured → outside', () => {
    expect(detectOutsideKillzone({ entryTs: Date.UTC(2026, 3, 20, 8, 30), killzones: [] })).toBe(
      true,
    )
  })
})
...
describe('detectCircuitBreakerBypassed', () => {
  it('not flagged when the session was never locked', () => {
    expect(detectCircuitBreakerBypassed({ sessionLockedAt: null, tradeCreatedAt: 2000 })).toBe(
      false,
    )
  })
  ...
  it('not flagged when the lock came after the trade (this trade tripped it)', () => {
    expect(detectCircuitBreakerBypassed({ sessionLockedAt: 2000, tradeCreatedAt: 1000 })).toBe(
      false,
    )
  })
