...
describe('urgencyHurts', () => {
  ...
  it('returns null when delta ≤ 10 percentage points', () => {
    ...
    const trades = [
      ...fill(6, 4, true),
      ...fill(4, 4, false), // 60% win at low urgency
      ...fill(6, 8, true),
      ...fill(4, 8, false), // 60% win at high urgency
    ]
    expect(urgencyHurts(trades)).toBeNull()
  })
  ...
  it('returns insight when delta > 10pp and both buckets ≥ 10', () => {
    ...
    const trades = [
      ...fill(8, 3, true),
      ...fill(2, 3, false), // low urg: 80%
      ...fill(3, 9, true),
      ...fill(7, 9, false), // high urg: 30%
    ]
  ...
  it('uses medium severity when delta is 11–20 pp', () => {
    ...
    const trades = [
      ...
      ...fill(3, 3, false), // 70%
      ...fill(6, 9, true),
      ...fill(4, 9, false), // 60% — actually 60, not 55
    ]
  ...
  it('urgency 6 is neither low (≤5) nor high (≥7) — ignored', () => {
    const trades = [
      ...fill(8, 4, true),
      ...fill(2, 4, false), // low urgency: 80%
      ...fill(10, 6, false), // neutral urgency 6
      ...fill(3, 8, true),
      ...fill(7, 8, false), // high urgency: 30%
    ]
    const result = urgencyHurts(trades)
    expect(result).not.toBeNull()
