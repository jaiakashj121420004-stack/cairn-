  ...
  it('fires rule.violated for each broken rule', () => {
    ...
    db.insert(schema.trades)
      .values({
        ...
        status: 'open',
        entryPrice: 109_000_0,
        stopLossPrice: 109_100_0,
        takeProfitPrice: 108_700_0,
        slPips: 100,
        rrRatio: 300,
        lotSize: 5,
        riskAmountCents: 50_00,
        riskPctBps: 50,
    ...
    const keys = violations.map(
      ([, ev]: [string, { name: string; payload: { ruleKey: string } }]) => ev.payload.ruleKey,
    )
