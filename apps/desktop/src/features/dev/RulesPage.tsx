...
export function RulesPage() {
  ...
  useEffect(() => {
    void (async () => {
      const [accRes, rulesRes] = await Promise.all([ipc.accounts.list(), ipc.rules.listAvailable()])
      if (accRes.ok) {
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card className="space-y-4 p-4">
          ...
          {draft && (
            <div className="grid grid-cols-3 gap-3">
              <NumField
                label="rrRatio (×100)"
                value={draft.rrRatio}
                onChange={(v) => setDraft({ ...draft, rrRatio: v })}
              />
              <NumField
                label="riskPctBps"
                value={draft.riskPctBps}
                onChange={(v) => setDraft({ ...draft, riskPctBps: v })}
              />
              <NumField
                label="riskAmountCents"
                value={draft.riskAmountCents}
                onChange={(v) => setDraft({ ...draft, riskAmountCents: v })}
              />
              <NumField
                label="lotSize"
                value={draft.lotSize}
                onChange={(v) => setDraft({ ...draft, lotSize: v })}
              />
              <NumField
                label="slPips"
                value={draft.slPips}
                onChange={(v) => setDraft({ ...draft, slPips: v })}
              />
              <NumField
                label="mssConfirmed (0/1)"
                value={draft.mssConfirmed}
                onChange={(v) => setDraft({ ...draft, mssConfirmed: v })}
              />
              <NumField
                label="htfBiasAligned (0/1)"
                value={draft.htfBiasAligned}
                onChange={(v) => setDraft({ ...draft, htfBiasAligned: v })}
              />
              <NumField
                label="preCalmScore"
                value={draft.preCalmScore}
                onChange={(v) => setDraft({ ...draft, preCalmScore: v })}
              />
              <NumField
                label="preUrgencyScore"
                value={draft.preUrgencyScore}
                onChange={(v) => setDraft({ ...draft, preUrgencyScore: v })}
              />
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" onClick={() => accountId && void refreshSession(accountId)}>
