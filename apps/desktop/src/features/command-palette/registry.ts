...
// react-router v7's NavigateFunction returns a Promise; these commands fire
// navigation without awaiting it, so each call is explicitly voided (§19.3).
...
export const STATIC_COMMANDS: readonly CommandDef[] = [
  ...
  },
  {
    ...
    action: ({ navigate, closeCommandPalette }) => {
      closeCommandPalette()
      void navigate('/notebook?action=new')
    },
  },
  {
    id: 'search-notebook',
    ...
    action: ({ navigate, closeCommandPalette }) => {
      closeCommandPalette()
      void navigate('/notebook?action=search')
    },
  },
  {
    id: 'export-pdf',
...
export function buildAccountCommands(
  ...
  return accounts.map((a) => ({
    ...
    action: ({ navigate, closeCommandPalette }) => {
      closeCommandPalette()
      void navigate(`/dashboard?account=${a.id}`)
    },
  }))
}

/** Build one command per playbook: "New trade from playbook: <name>" */
export function buildPlaybookCommands(playbooks: Playbook[]): CommandDef[] {
...
export const SETTINGS_TAB_COMMANDS: readonly CommandDef[] = [
  {
    ...
    keywords: ['settings', 'general', 'timezone', 'leverage', 'risk'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('general')
      void navigate('/settings')
    },
  },
  {
    id: 'settings-pairs',
    label: 'Settings: Pairs',
    keywords: ['settings', 'pairs', 'instruments', 'symbols'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('pairs')
      void navigate('/settings')
    },
  },
  {
    id: 'settings-setups',
    label: 'Settings: Setups',
    keywords: ['settings', 'setups', 'strategies'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('setups')
      void navigate('/settings')
    },
  },
  {
    id: 'settings-killzones',
    label: 'Settings: Killzones',
    keywords: ['settings', 'killzones', 'sessions', 'london', 'ny'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('killzones')
      void navigate('/settings')
    },
  },
  {
    id: 'settings-firms',
    label: 'Settings: Prop Firms',
    keywords: ['settings', 'firms', 'prop', 'ftmo', 'rules'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('firms')
      void navigate('/settings')
    },
  },
  {
    ...
    keywords: ['settings', 'alerts', 'r-target', 'notifications'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('alerts')
      void navigate('/settings')
    },
  },
  {
    id: 'settings-data',
    label: 'Settings: Data',
    keywords: ['settings', 'data', 'import', 'export'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('data')
      void navigate('/settings')
    },
  },
  {
    ...
    keywords: ['settings', 'backup', 'restore'],
    action: ({ navigate, setSettingsTabRequested, closeCommandPalette }) => {
      closeCommandPalette()
      setSettingsTabRequested('backups')
      void navigate('/settings')
    },
  },
] as const
