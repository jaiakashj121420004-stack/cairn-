...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h3 className="mb-3 text-body font-semibold text-text-primary">{children}</h3>
}
          ...
          <ResponsiveContainer>
            <BarChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <YAxis
                ...
                tickFormatter={(v) => formatCents(v as number)}
                tick={{ fontSize: 11 }}
              />
          ...
          <ResponsiveContainer>
            <BarChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="winRate" name="Win rate">
                {chartData.map((d, i) => (
                  <Cell key={i} fill={d.winRate >= 50 ? CHART_COLORS.win : CHART_COLORS.loss} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <ResponsiveContainer>
        <BarChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <ReferenceLine
            ...
            strokeWidth={1.5}
            label={{ value: '0', position: 'top', fontSize: 11 }}
          />
        ...
        <StatCard
          ...
          value={
            data.dowSummary.length > 0
              ? (DOW_LABELS[
                  data.dowSummary.reduce((best, r) => (r.netPnlCents > best.netPnlCents ? r : best))
                    .dow
                ] ?? '—')
              : '—'
          }
