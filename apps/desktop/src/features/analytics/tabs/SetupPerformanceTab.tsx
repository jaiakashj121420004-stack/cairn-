...
export function SetupPerformanceTab() {
  ...
  useEffect(() => {
    setLoading(true)
    void Promise.all([ipc.analytics.setups(filter), ipc.analytics.playbookStats(filter)]).then(
      ([r, pb]) => {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-surface-elevated p-4">
        ...
        <div className="h-64">
          <ResponsiveContainer>
            <BarChart
              data={data.bySetup.map((r) => ({
                ...r,
                label: setupNameMap.get(r.setupId) ?? r.setupId,
              }))}
            >
        ...
        <div className="h-48">
          <ResponsiveContainer>
            <BarChart data={data.byDay.map((r) => ({ ...r, label: DOW_LABELS[r.dow] ?? r.dow }))}>
              <CartesianGrid stroke={CHART_COLORS.grid} strokeDasharray="3 3" />
              <XAxis dataKey="label" stroke={CHART_COLORS.axis} />
              <YAxis stroke={CHART_COLORS.axis} tickFormatter={(v) => `${Number(v) / 100}%`} />
                ...
                <tr key={r.playbookId} className="border-t border-border">
                  ...
                  <td
                    className={cn(
                      ...
                      r.expectancyR > 0 ? 'text-accent-a' : 'text-danger',
                    )}
                  >
