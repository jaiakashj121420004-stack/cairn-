...
export function AccountsPage() {
  ...
  async function openCreate() {
    ...
    if (!firmId) {
      ...
      if (!res.ok) {
        toast('Could not create a default firm. Go to Settings → Prop Firms first.', 'error')
        return
      }
      await load()
      firmId = res.data.id
  ...
  return (
    <div className="flex h-full flex-col">
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-caption text-text-muted">
                      {firmMap.get(account.propFirmId) ?? '—'} · Phase {account.currentPhase}/
                      {account.stepCount} · {fmtMoney(account.accountSizeCents)}
                    </p>
                  </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-caption text-text-muted">
            This permanently removes the account and all its trades, sessions, and rule history.
