  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([rulesRes, detectRes]) => {
        ...
        if (items.length > 0)
          setRulesBroken((prev) => Array.from(new Set([...prev, ...items.map((d) => d.ruleKey)])))
      })
        ...
        <div className="mb-1 grid grid-cols-3 gap-3 rounded-[10px] border border-border bg-surface px-4 py-3 text-center">
          <div>
            ...
            <p
              className={cn(
                ...
                (trade.pnlCents ?? 0) >= 0 ? 'text-accent-a' : 'text-danger',
              )}
            >
          ...
          <div>
            ...
            <p
              className={cn(
                'font-mono text-body-sm font-semibold',
                (trade.pnlR ?? 0) >= 0 ? 'text-accent-a' : 'text-danger',
              )}
            >
          ...
          <div>
            ...
            <p className="font-mono text-body-sm text-text-primary capitalize">
              {trade.exitReason ?? '—'}
            </p>
          </div>
        </div>
