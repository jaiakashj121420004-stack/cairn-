...
export function TradeDetailModal({ tradeId, onClose, onTradeUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    void ipc.trades.get(tradeId).then((res) => {
      setLoading(false)
                        ...
                        {detail.screenshots.map((ss) => (
                          ...
                          >
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-background/70 px-2 py-1 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                              <span className="text-micro text-text-secondary capitalize">
                                {ss.kind.replace('_', ' ')}
                              </span>
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  void handleDeleteScreenshot(ss.id)
                                }}
                            ...
                            >
                              <div>
                                ...
                                <p className="font-mono text-text-primary">
                                  {pc.closeLots !== null
                                    ? (pc.closeLots / 100).toFixed(2)
                                    : `${(pc.closePercentBps / 100).toFixed(0)}%`}
                                </p>
                              </div>
                              <div>
                                <p className="text-text-muted">Exit</p>
                                <p className="font-mono text-text-primary">
                                  {exitFloat.toFixed(detail.pairPipDecimal)}
                                </p>
                              </div>
                              <div>
                                ...
                                <p
                                  className={cn(
                                    'font-mono font-semibold',
                                    (pc.pnlCents ?? 0) >= 0 ? 'text-accent-a' : 'text-danger',
                                  )}
                                >
