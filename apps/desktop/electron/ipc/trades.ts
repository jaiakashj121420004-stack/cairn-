...
function mapScreenshotRow(row: typeof schema.tradeScreenshots.$inferSelect): TradeScreenshot {
...
export function registerTradeHandlers(): void {
    ...
    try {
      ...
      if (nearDup) {
        return {
          ...
          error: {
            ...
            message:
              'A trade with the same account, pair, and direction was logged within the last 5 minutes. Check your trade log before proceeding.',
          },
        }
      }
      ...
      db.transaction(() => {
        ...
        if (d.status === 'open' && d.sessionId) {
          db.update(schema.sessions)
            ...
            .where(and(eq(schema.sessions.id, d.sessionId), isNull(schema.sessions.lockedAt)))
            .run()
        }
      })
      ...
      if (!e.sender.isDestroyed()) {
        e.sender.send('cairn:event', {
          name: 'trade.placed',
          payload: { tradeId: id, accountId: d.accountId },
        })
        if (d.status === 'open' && d.sessionId) {
          e.sender.send('cairn:event', {
            name: 'session.locked',
            payload: { accountId: d.accountId },
          })
        }
      }
  ...
  ipcMain.handle('trades:close', (e, raw: CloseTradeInput): IpcResponse<Trade> => {
    ...
    try {
      ...
      if (!e.sender.isDestroyed()) {
        e.sender.send('cairn:event', {
          name: 'trade.closed',
          payload: { tradeId: d.tradeId, accountId: trade.accountId, pnlCents },
        })
        for (const ruleKey of d.rulesBroken) {
          e.sender.send('cairn:event', {
            name: 'rule.violated',
            payload: { accountId: trade.accountId, ruleKey, tradeId: d.tradeId },
          })
        }
      }
      return { ok: true, data: mapRow(row) }
    ...
    try {
      ...
      if (trade.status !== 'closed') {
        return {
          ...
          error: {
            code: 'CONFLICT',
            message: 'Trade is not closed; reflection applies to closed trades only',
          },
        }
      }
      if (trade.phase2Complete === 1) {
        return {
          ok: false,
          error: { code: 'CONFLICT', message: 'Reflection already completed for this trade' },
        }
      }
      ...
      if (!e.sender.isDestroyed()) {
        e.sender.send('cairn:event', {
          ...
          payload: { tradeId: d.tradeId, accountId: trade.accountId },
        })
        for (const ruleKey of d.rulesBroken) {
          e.sender.send('cairn:event', {
            name: 'rule.violated',
  ...
  ipcMain.handle('trades:partialClose', (e, raw: PartialCloseInput): IpcResponse<Trade> => {
    try {
      ...
      if (raw.closeLots <= 0 || raw.closeLots >= trade.lotSize) {
        return {
          ...
          error: {
            ...
            message: 'closeLots must be positive and less than current lot size',
          },
        }
      }
      ...
      const pnlR =
        trade.slPips > 0
          ? Math.round(
              ((raw.exitPrice - trade.entryPrice) * (trade.direction === 'long' ? 1 : -1) * 100) /
                trade.slPips,
            )
          : null
      ...
      if (!e.sender.isDestroyed()) {
        e.sender.send('cairn:event', {
          ...
          payload: { tradeId: raw.tradeId, accountId: trade.accountId },
        })
      }
      return { ok: true, data: mapRow(row) }
    } catch (err) {
  ...
  ipcMain.handle('trades:list', (_e, raw: TradeFilter): IpcResponse<TradeListItem[]> => {
    ...
    try {
      ...
      const conditions = [eq(schema.trades.accountId, d.accountId), isNull(schema.trades.deletedAt)]
      ...
      if (d.isClean !== undefined) conditions.push(eq(schema.trades.isClean, d.isClean ? 1 : 0))
    ...
    (_e, raw: { screenshotId: string }): IpcResponse<{ ok: true }> => {
      try {
        ...
        const row = db
          ...
          .get()
        if (!row)
          return { ok: false, error: { code: 'NOT_FOUND', message: 'Screenshot not found' } }
