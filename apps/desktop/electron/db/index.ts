...
function createPreMigrationBackup(dbPath: string, dbDir: string): void {
  try {
    ...
    const backupPath = join(backupDir, `pre-migration-${ts}.sqlite`)
    copyFileSync(dbPath, backupPath)
    log.info(`[db] Pre-migration backup created: ${backupPath}`)
  } catch (err) {
    // Non-fatal — migration proceeds even without the safety snapshot.
