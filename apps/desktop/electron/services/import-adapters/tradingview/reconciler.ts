...
function reconcileGroup(tradeNum: string, rows: RawTvRow[]): ImportCandidate | null {
  const entryRow = rows.find((r) => isEntry(r.type))
  if (!entryRow) return null // orphan exits — nothing to import

  const exitRows = rows.filter((r) => isExit(r.type)).sort((a, b) => a.dateTimeMs - b.dateTimeMs)
  ...
  const totalPnl = exitRows.reduce((sum, r) => sum + safeFloat(r.profit), 0)
