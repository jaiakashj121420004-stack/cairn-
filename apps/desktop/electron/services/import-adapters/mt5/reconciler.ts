 ...
 */
export function reconcileDeals(deals: RawDeal[], orders: RawOrder[]): Mt5TradeCandidate[] {
...
function reconcileGroup(
  ...
  const entryDeal = sorted.find((d) => d.dealDirection === 'in' || d.dealDirection === 'in/out')
  ...
  const totalComm = [entryDeal, ...exitDeals].reduce((s, d) => s + safeFloat(d.commission), 0)
  const totalSwap = exitDeals.reduce((s, d) => s + safeFloat(d.swap), 0)

  const direction: TradeDirection = entryDeal.dealType === 'buy' ? 'long' : 'short'
