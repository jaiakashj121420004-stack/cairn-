...
export function calcRiskCents(slTenths: number, lotSizeInt: number, pipValueCents: number): number {
  return slTenths > 0 ? Math.round((slTenths * lotSizeInt * pipValueCents) / 1000) : 0
}
