...
**Current status (2026-07-29):** Root-caused why commit `04895b7` (the corrected brace-expansion override) still failed CI #12 / Release #7 / security #13: **`pnpm-lock.yaml` was never regenerated after two later package.json override edits** — every job in all three workflows opens with `pnpm install --frozen-lockfile`, which fails fast with `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH` the moment the lockfile's overrides header and package.json diverge (confirmed directly from the Actions job logs, not guessed). Regenerated the lockfile for real (worked around two sandbox-only blockers — a filesystem `unlink()` restriction on the mounted repo dir, and blocked egress on one git-tarball dependency — by running `pnpm install --no-frozen-lockfile` in the mounted dir anyway, since pnpm writes the corrected lockfile before it reaches the step that crashes; verified by diffing against HEAD and re-running `--frozen-lockfile` against the result, which now passes cleanly). Diff is minimal and correct: brace-expansion override narrows as intended, the vulnerable `minimatch@10.2.5` chain stays patched, and the three ESLint-adjacent `minimatch` chains (`3.1.5`/`5.1.9`/`9.0.9`) revert to their real pre-override versions — fully reversing the earlier blanket-override ESLint breakage. Committed as `16684d4` on `feat/rules-engine`, **NOT yet pushed** (no push credentials in Claude's sandbox). **Akash's only remaining step: `git push origin feat/rules-engine`** — no new tag needed, `v0.2.1` already points at `04895b7`. Full detail: `docs/build-status.md` § "2026-07-29 — Root-caused and fixed the CI/Release/security failures on commit 04895b7". _Prior status retained below._
