...
**Current status (2026-06-05):** v1.2 feature-complete (Waves 0–3, items 1–24). v2.0 cloud rebuild in progress: Stage 18.4 client crypto (`e812589`) and Stage 18.5/18.6 server (`91c0fb2`, vault key-material + password reset `15299b3`) committed. **Stage 18.5 (Stage 3) is now feature-complete** — the last outstanding scope item, OpenAPI 3.1 spec + self-hosted Scalar API reference (`GET /openapi.json`, `GET /docs`, env-gated), landed this cycle with 6 DB-free docs tests (full detail in `docs/build-status.md`). The Stage 18.6 **client sync slice** (encryption-on-write + push/pull/merge for the `trades` table only) is built and committed (`363f8c8`) — see `docs/build-status.md` for per-stage detail and the known scope boundary (registering the other syncable tables, main-process sync wiring `SyncWriteContext`/`createSyncRunner`, and the conflict-resolution modal are the next prompt). Gates last reported green by the build session (typecheck · lint 0 · 791 unit tests · build; smoke E2E not re-run). The v0.1.1 packaged-migrations hotfix (`d53be8d`) is committed, repackaged (2026-06-07, `Cairn Setup 0.1.0.exe`), and clean-install verified (2026-06-11) — asar confirmed all 14 migrations present, app launched to onboarding with no migration error (full detail in `docs/build-status.md`). **Wave 4 (live broker)** is built but uncommitted on `feat/vault-enrollment-server`: the statement↔live reconciliation rule (#35) and the streamed-fill `enqueueSyncOp` sync-gap (#36) are done & tested this cycle (desktop gates green); the account-map UI and cTrader protobuf codec are the two remaining seams before live MT5/cTrader end-to-end (see `docs/build-status.md`). **Stage 7 (production hardening & launch)** has its release pipeline wired this cycle, uncommitted: code-signing/notarization config in `electron-builder.yml` (inert until certs are supplied), `.github/workflows/release.yml`, and `docs/launch-checklist.md` — see `docs/build-status.md` for detail. Signing secrets, the security/legal Stage 7 prompts, and the soft-launch window itself are not yet started.
