...
| `docs/react-router-v8-migration-plan.md` | **[2026-07-29 NEW, NOT STARTED]** — binding plan for a deferred, coordinated toolchain bump: Node 20→22.22+, React 18→19.2.7+, Vite 5/6→7+, `react-router-dom`→`react-router` v8 (the only real fix for 2 Dependency Audit findings). Full file-by-file import list, breaking-change checklist, sandbox lockfile-regen gotchas, and a mandatory UI/UX + feature manual-QA checklist. Read this whole file before starting that work. |
...
**Current status (2026-07-29):** Root-caused why commit `04895b7` (the corrected brace-expansion override) still failed CI #12 / Release #7 / security #13: **`pnpm-lock.yaml` was never regenerated after two later package.json override edits** — every job in all three workflows opens with `pnpm install --frozen-lockfile`, which fails fast with `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH` the moment the lockfile's overrides header and package.json diverge (confirmed directly from the Actions job logs, not guessed). Regenerated the lockfile for real and Akash pushed it (`16684d4` + docs `aaeb2fb`, pushed as `04895b7..aaeb2fb`). **That fix worked** — every job now gets past `pnpm install` for the first time in this saga (Build/License Check/Unit Tests all pass on CI run `30470978607`) — but it unblocked 3 more real, previously-hidden failures, all confirmed from the actual job logs and now fixed: (1) Prettier formatting on 6 hand-crafted marketing/legal HTML files, fixed via `.prettierignore` rather than reformatting (risk of disturbing embedded JS/legal text) — `3caf057`; (2) Smoke E2E's Playwright install still failed (`@playwright/test` lives in `apps/desktop`, not root — bare `pnpm exec` at root can't find it), fixed by scoping with `--filter @cairn/desktop` — `3caf057`; (3) security.yml's two Trivy jobs failed on a `trivy-action@0.28.0` pin that never resolved (no such tag has ever existed upstream, checked directly) — bumped to the current latest `v0.36.0` — `20a4636`. **Not yet pushed.** Dependency Audit's react-router finding (2 high) is deliberately still untouched — Akash's call on 2026-07-29 was to do the **real** fix (not a suppression) but as its own dedicated future session, not bundled into this one. Turns out the real fix is much bigger than a version bump: `react-router@8.3.0` (confirmed via the npm registry directly) requires Node ≥22.22.0 and React ≥19.2.7 as hard prerequisites, and also chains into a Vite 5/6→7+ bump — this repo is on Node 20 (`.nvmrc`, every CI job) and React 18.3.1. Full binding plan, file-by-file import list, and a mandatory UI/UX + feature manual-QA checklist: **`docs/react-router-v8-migration-plan.md` (NEW, NOT STARTED)** — read that whole file before starting that work. Full detail on today's CI fixes: `docs/build-status.md` § "2026-07-29 — Root-caused and fixed the CI/Release/security failures on commit 04895b7" and its same-day addendum. _Prior status retained below._

**Prior status (2026-07-28):** Live QA on a real MT5 trade surfaced a real bug — live-captured broker fills (MT5/cTrader) wrote **$0.00 P&L** to Dashboard/Analytics (pnlR was already correct; only the dollar figure was stubbed). Fixed in `committer.ts`/`accumulator.ts`: live rows now derive $ P&L from price data via the existing tested `calculatePnl()`, gated so statement imports are untouched (`fix(broker)` `4cf406f`). While re-verifying, found a **separate, pre-existing** precision bug in `pnl-calculator.ts`'s `roundToInt()` — silently lost precision past `Number.MAX_SAFE_INTEGER` on a synthetic degenerate input; now throws instead of silently corrupting, plus a realistic test-bound fix (`fix(pnl)` `6e43750`). Also committed: the elaborate marketing/features site (`docs/cairn-app-guide.html`) and MT5/cTrader connection guide + interactive checklist (`docs/connect-mt5-and-ctrader.{md,html}`), plus previously-drafted legal pages (`legal/*.html`). **Addendum, same day:** pushed the branch + tag `v0.2.1` — both `CI` and `Release` workflows failed. Root cause: `pnpm/action-setup@v4` on `version: latest` now resolves to pnpm 11.x, which dropped Node 20 support; every job in `ci.yml`/`release.yml`/`backup-verify.yml` still runs Node 20 (`.nvmrc`), so `pnpm install` crashed with `ERR_UNKNOWN_BUILTIN_MODULE` on **every run since at least commit `efef4cc`** — CI has been silently red for a while, and neither v0.2.0 nor v0.2.1 ever built via this pipeline (v0.2.0's GitHub release was published manually). Pinned all three to `10.33.0` (matching `security.yml`'s already-working pin) in commit `a1c0d4d`. **5 commits on `feat/rules-engine` now, working tree clean, NOT YET PUSHED** — branch is 34 commits ahead of `origin/feat/rules-engine` (Claude's sandbox has no push credentials). **Verification gaps:** (1) `typecheck`/`test:unit` not re-run on host since the `fix(pnl)` commit; (2) the CI pin fix itself hasn't been proven green yet — needs push + tag `v0.2.1` recreated (it points at the pre-fix commit) + a fresh Actions run. **Update: pin fix pushed, and it worked — Unit Tests went green on a clean CI Windows runner (closes gap #1 for real), but uncovered 3 more independent bugs underneath the pnpm crash:** a Modal.tsx git-casing mismatch (`fix(desktop)` `0f824a2`), a wrong artifact path breaking Smoke E2E (`fix(ci)` `3682a5c`), and 7 of 8 stale/missing dependency-audit overrides (`fix(deps)` `02c9cf1` — react-router's 2 remaining advisories need a major-version bump, deliberately deferred, not silently ignored). **`pnpm-lock.yaml` still needs regenerating locally before the next push** (sandbox can't complete `pnpm install` — blocked egress on a git-tarball dep) or `--frozen-lockfile` fails again. Full detail: `docs/build-status.md` § "2026-07-28 — Live-capture $0 P&L bug fix + pnl-calculator precision guard + marketing/broker-guide docs" and its two same-day addenda. _Prior status retained below._

**Prior status (2026-07-12):** **P4 Dodo Payments billing COMPLETE — gates GREEN on `feat/rules-engine`** (typecheck / lint / test:unit / build / e2e / `@cairn/server` full suite all PASS with Postgres up; only the pre-existing, unrelated `docs routes smoke` red — the `ENABLE_API_DOCS=false` path, failed identically before/after P4). Dodo is an **additive** `BillingProvider` (Stripe/Razorpay untouched) and the **default gateway for every country** when configured (`selectBillingProvider`): Merchant-of-Record handling India GST + international tax. Price **$15/mo · $150/yr** (₹1,299 / ₹12,990 GST-incl.). Shipped: `billing/dodo-signature.ts` (Standard Webhooks HMAC, 5-min replay tolerance), `billing/dodo-provider.ts`, migration `0005_dodo_webhook_provider.sql` (widen `webhook_event.provider` CHECK), `POST /webhooks/dodo` + `dispatchDodoEvent` (verify→dedupe→§20.5→invalidate), `DODO_*` env + `.env.example`, price copy, `docs/billing.md`/`docs/deployment.md`/§14 #23. Mandatory §2.13 signature + idempotency + refund tests pass. Full detail: `docs/build-status.md` § "2026-07-12 — P4 Dodo Payments billing COMPLETE". _Prior status retained below._
