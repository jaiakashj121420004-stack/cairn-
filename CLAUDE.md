...
**Current status (2026-06-05):** v1.2 feature-complete (Waves 0–3, items 1–24). v2.0 cloud rebuild in progress: Stage 18.4 client crypto (`e812589`) and Stage 18.5/18.6 server (`91c0fb2`) committed. The Stage 18.6 **client sync slice** (encryption-on-write + push/pull/merge for the `trades` table only) is built and committed in this cycle — see `docs/build-status.md` for per-stage detail and the known scope boundary (registering the other syncable tables, main-process sync wiring `SyncWriteContext`/`createSyncRunner`, and the conflict-resolution modal are the next prompt). Gates last reported green by the build session (typecheck · lint 0 · 791 unit tests · build; smoke E2E not re-run). The v0.1.1 packaged-migrations hotfix (`d53be8d`) is committed but not yet repackaged/reinstalled. Wave 4 (live broker) remains out of scope.
